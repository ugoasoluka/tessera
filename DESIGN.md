...
`terraform/bootstrap` provisions the S3 state bucket and DynamoDB lock table that `terraform/main` uses as its backend — solving the chicken-and-egg of remote state. The state bucket is versioned (rollback for accidental destroys), encrypted at rest with AES256, has all four public-access blocks enabled, and is paired with DynamoDB for state locking (prevents concurrent applies from corrupting state).

Each module's variables are deliberately conservative: `force_destroy = true` is exposed only on the bootstrap module and only used in the demo `tfvars`; production environments would set it to `false`.
...
EKS 1.35 with a managed node group of two `t3.medium` instances on `AL2023_x86_64_STANDARD`, with a launch template that explicitly attaches the custom worker security group (`vpc_security_group_ids = [aws_security_group.eks_worker.id]`). Without that, EKS falls back to attaching only its auto-cluster SG to nodes — which silently breaks any RDS or data-tier ingress rule that references the worker SG. (Worth calling out because I hit it: pods couldn't reach RDS until the worker SG was actually on the node ENIs.)
...
A GitHub OIDC provider is registered in AWS IAM. An IAM role (`tessera-gha-ecr-push`) trusts it with a `repo:ugoasoluka/tessera:*` subject condition, scoped to the minimum ECR push verbs (`BatchCheckLayerAvailability`, `InitiateLayerUpload`, `UploadLayerPart`, `CompleteLayerUpload`, `PutImage`, `BatchGetImage`). The build workflow assumes the role at runtime — no static AWS credentials in GitHub secrets, no long-lived access keys to rotate.

ECR repos are `IMMUTABLE` so re-pushing the same tag is a hard error rather than a silent overwrite. `scan_on_push = true` runs the AWS Inspector CVE scan on every image automatically. Daily versioning via `fregante/daily-version-action` produces tags like `v26.4.26`, `v26.4.26.1`, `v26.4.26.2` for same-day re-runs.
...
The rubric calls out concurrent users with privacy and session isolation. Mechanisms:
...
- **EKS access is single-identity.** Only the IAM principal that ran `terraform apply` has cluster admin via the default EKS auth mode. Production would use [EKS Access Entries](https://docs.aws.amazon.com/eks/latest/userguide/access-entries.html) (or `aws-auth` ConfigMap) to grant kubectl to additional IAM roles — on-call team, SRE rotation.
...
- **GitHub OIDC trust is wildcard.** `repo:ugoasoluka/tessera:*` allows any branch / PR / tag to assume the role. Production should scope to `:ref:refs/heads/main` or a GitHub Environment.
...
4. **Tighter GitHub OIDC trust policy** — restrict `sub` to `repo:ugoasoluka/tessera:ref:refs/heads/main` or to a named GitHub Environment with required reviewers.
5. **EKS Access Entries** for on-call / SRE / read-only auditor roles.
6. **Cluster autoscaler or Karpenter** for node-level elasticity. Spot node group as a second pool for non-latency-critical workloads.
7. **HPA + PodDisruptionBudget** on the temporal-worker deployment once load characteristics are known.
8. **Observability stack.** Prometheus + Grafana + Tempo (or any OTel-compatible collector). Pydantic AI + Temporal both expose OTel hooks already.
9. **EBS CSI / VPC CNI / kube-proxy / coredns version pins** in Terraform; currently using EKS-managed defaults which can drift on cluster upgrade.
10. **Per-user GitHub App OAuth** — replaces the single org-level PAT so PRs are attributable to the requesting human.

---

## Application runtime notes

A few software-side choices with deployment implications, kept short on purpose:
...
- **LLM provider is configurable.** Currently `anthropic:claude-haiku-4-5-20251001` (~$0.05–$0.15 per agent run). Switching providers is a one-line change in `helm/temporal-worker/values.yaml` plus an env-var rename in the `ExternalSecret`.
